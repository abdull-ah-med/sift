...
from sift_parse.datamodel.service.tasks import TaskProcessingMeta, TaskType
from sift_parse.utils.profiling import ProfilingItem

if TYPE_CHECKING:
    from sift_parse.datamodel.base_models import ConfidenceReport, PageConfidenceScores
...
class ConfidenceScores(BaseModel):
    """JSON-safe snapshot of sift_parse confidence scores.

    Plain stored floats (NaN coerced to null) rather than a reuse of
    sift_parse's ``ConfidenceReport``, so the wire schema carries no numpy
