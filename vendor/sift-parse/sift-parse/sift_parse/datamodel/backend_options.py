...
class PdfBackendOptions(BaseBackendOptions):
    ...
    enforce_same_font: bool = Field(
        True,
        description=(
            "Whether sift-parse-pdf should split text cells at font boundaries. "
...
class ThreadedDoclingParseBackendOptions(PdfBackendOptions):
    """Options specific to the threaded sift-parse-pdf backend."""

    kind: Literal["threaded-sift-parse-pdf"] = Field(
        "threaded-sift-parse-pdf", exclude=True, repr=False
    )
    parser_threads: Optional[PositiveInt] = Field(
        None,
        description=(
            "Number of parser threads to use for the threaded sift-parse-pdf backend. "
    ...
    release_native_memory_every_n_pages: conint(ge=0) = Field(
        ...
        description=(
            ...
            "threaded sift-parse-pdf backend. Set to 0 to disable native-memory "
            "release."
        ),
    )
