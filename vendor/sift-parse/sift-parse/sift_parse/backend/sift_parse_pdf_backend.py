...
from sift_parse_core.types.doc import BoundingBox, CoordOrigin, Size
from sift_parse_core.types.doc.page import SegmentedPdfPage, TextCell
...
from sift_parse.backend.managed_pdfium_backend import (
    ManagedPdfiumDocumentBackend,
    ManagedPdfiumPageBackend,
)
from sift_parse.backend.pdf_backend import PdfDocumentBackend, PdfPageBackend
from sift_parse.datamodel.accelerator_options import AcceleratorOptions
from sift_parse.datamodel.backend_options import (
    PdfBackendOptions,
    ThreadedDoclingParseBackendOptions,
)
from sift_parse.datamodel.settings import DEFAULT_PAGE_RANGE
from sift_parse.exceptions import DocumentLoadError
from sift_parse.utils.locks import pypdfium2_lock
from sift_parse.utils.pdf_outline import (
    _PdfOutlineItem,
...
class DoclingParsePageBackend(ManagedPdfiumPageBackend):
    ...
    def get_size(self) -> Size:
        ...
        # TODO: Take width and height from sift-parse-pdf.
        # return Size(
...
class DoclingParseDocumentBackend(ManagedPdfiumDocumentBackend):
    def __init__(
        ...
        try:
            ...
            # pypdfium2 (PdfiumError) and sift-parse-pdf both signal unreadable
            # bytes by raising RuntimeError; tag it as a load failure.
            detail = str(e).strip()
            if detail:
                raise DocumentLoadError(
                    f"sift-parse-pdf could not load document {self.document_hash}: {detail}"
                ) from e
            raise DocumentLoadError(
                f"sift-parse-pdf could not load document {self.document_hash}."
            ) from e
    ...
    def page_count(self) -> int:
        # return len(self._pdoc)  # To be replaced with sift-parse-pdf API
    ...
    def get_document_outline(self) -> list[_PdfOutlineItem]:
        """Extract the outline via sift-parse-pdf's native table-of-contents (no pypdfium2)."""
        if self.dp_doc is None:
            return []
...
class ThreadedDoclingParseDocumentBackend(PdfDocumentBackend):
    ...
    def get_document_outline(self) -> list[_PdfOutlineItem]:
        """Extract the outline via sift-parse-pdf (this backend holds no pypdfium2 handle).

        The threaded parser exposes no table-of-contents accessor, so a lightweight lazy
        sift-parse-pdf document is loaded purely to read the (cheap, structure-only) outline.
        """
        password = (
            self.options.password.get_secret_value() if self.options.password else None
        )
