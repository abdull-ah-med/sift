...
from sift_parse_core.types.doc.document import Formatting, Script
...
_INSTALL_HINT = (
    ...
    "Install them with `pip install 'sift-parse[format-xml-jats]'`."
)
...
# Maps JATS formatting tags to sift-parse-core formatting attributes.
...
class JatsDocumentBackend(DeclarativeDocumentBackend):
    ...
    def _get_text(node: etree._Element, sep: str | None = None) -> str:
        ...
        for child in list(node):
            if child.tag not in skip_tags:
                # TODO: apply styling according to child.tag when supported by sift-parse-core
    ...
    def _add_authors(self, doc: DoclingDocument, xml_components: XMLComponents) -> None:
        # TODO: once sift_parse supports text formatting, add affiliation reference to
