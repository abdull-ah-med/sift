...
from sift_parse_core.types.doc.document import ContentLayer, Formatting, ImageRef, Script
...
from sift_parse.backend.utils.image_resource_loader import ImageResourceLoader
from sift_parse.datamodel.backend_options import HTMLBackendOptions
...
from sift_parse.utils.code_language import (
    _HINT_PREFIXES,
...
_INSTALL_HINT = (
    ...
    "Install it with `pip install 'sift-parse[format-html]'`."
)
...
class HTMLDocumentBackend(DeclarativeDocumentBackend):
    ...
    def _render_with_browser(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Install it with 'pip install \"sift_parse[htmlrender]\"' and run "
                "'playwright install'."
            ) from exc
    ...
    def _handle_list(self, tag: Tag, doc: DoclingDocument) -> RefItem:
        ...
        for li in tag.find_all({"li", "ul", "ol"}, recursive=False):
            ...
            # addressing invalid HTML (sift-parse-core/issues/357)
            if li.name in {"ul", "ol"}:
