...
from sift_parse_core.types.doc.document import ContentLayer
...
from sift_parse.backend.docx.drawingml.utils import convert_to_modern_format
from sift_parse.datamodel.backend_options import MsPowerpointBackendOptions
from sift_parse.datamodel.base_models import FormatToMimeType, InputFormat
...
_INSTALL_HINT = (
    ...
    "Install it with `pip install 'sift-parse[format-pptx]'`."
)
...
class MsPowerpointDocumentBackend(DeclarativeDocumentBackend, PaginatedDocumentBackend):
    ...
    def _chart_type_to_classification(chart_type: Any) -> PictureClassificationLabel:
        """Map a python-pptx ``XL_CHART_TYPE`` to a sift_parse classification label.
