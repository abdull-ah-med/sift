...
* :func:`extract_outline_from_docling_parse` -- for the sift-parse-pdf backends, using their native
...
the ``sift_parse.service_client`` import chain. That chain must stay importable on any sift-parse
...
class _PdfOutlineItem(BaseModel):
    ...
    level: int
    # 1-based target page; None when the entry has no resolvable page (e.g. sift-parse-pdf ToC).
...
def extract_outline_from_docling_parse(
    ...
    """Flatten sift-parse-pdf's native table-of-contents into ordered ``_PdfOutlineItem``\\ s.
