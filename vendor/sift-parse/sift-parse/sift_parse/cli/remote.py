"""`sift_parse convert-remote` — convert documents through a docling-serve service.
...
from sift_parse.cli.export_utils import _export_flags_from_formats, _split_list
from sift_parse.cli.main import ChunkerType
from sift_parse.datamodel.base_models import InputFormat, OutputFormat
from sift_parse.datamodel.pipeline_options import ProcessingPipeline
from sift_parse.datamodel.service.options import (
    ConvertDocumentsOptions as ConvertDocumentsRequestOptions,
)
from sift_parse.datamodel.settings import PageRange
from sift_parse.service_client import (
...
identical to `sift_parse convert`. Only options the service honors are exposed here;
local-execution flags (device, threads, pdf-backend internals, debug visualizers)
do not apply to remote conversion and are intentionally absent.
"""
...
Examples:
  ...
  sift_parse convert-remote report.pdf

  # Explicit endpoint + key, Markdown and JSON output to ./out
  sift_parse convert-remote --service-url https://sift_parse.example.com \\
    --api-key "$KEY" --to md --to json --output ./out report.pdf

  # Whole directory, only PDFs and DOCX, no OCR
  sift_parse convert-remote --from pdf --from docx --no-ocr ./inbox
"""
...
def _collect_sources(
    ...
    from sift_parse.cli.main import (
        _is_http_url,
...
def convert_remote(
    ...
    from sift_parse.cli.main import err_console, export_documents
...
def register(app: "typer.Typer") -> None:
    ...
    Called from ``sift_parse.cli.main`` after the app and shared helpers are defined
    ...
    app.command(
        ...
        short_help=(
            ...
            "(see `sift_parse convert-remote --help`)."
        ),
