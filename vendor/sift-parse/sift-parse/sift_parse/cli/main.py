...
from sift_parse.datamodel.service.responses import ChunkedDocumentResultItem
...
try:
    ...
    print("\nThe sift_parse CLI requires additional dependencies.", file=sys.stderr)
    print("Please install them using one of the following options:\n", file=sys.stderr)
    print("  1. Install the full sift_parse package (recommended):", file=sys.stderr)
    print("     pip install sift_parse\n", file=sys.stderr)
    print("  2. Install sift-parse with CLI support:", file=sys.stderr)
    print("     pip install sift-parse[cli]\n", file=sys.stderr)
    print("  3. Install just the missing dependencies:", file=sys.stderr)
    print("     pip install typer rich\n", file=sys.stderr)
    sys.exit(1)

from sift_parse_core.transforms.serializer.html import (
    HTMLDocSerializer,
    HTMLOutputStyle,
    HTMLParams,
)
from sift_parse_core.transforms.visualizer.layout_visualizer import LayoutVisualizer
from sift_parse_core.types.doc import ImageRefMode
from sift_parse_core.utils.file import resolve_source_to_path
from pydantic import TypeAdapter
from rich.console import Console

from sift_parse.cli.export_utils import (
    _export_flags_from_formats,
    _is_empty_output,
    _should_generate_export_images,
    _split_list,
)
from sift_parse.datamodel.accelerator_options import AcceleratorDevice, AcceleratorOptions
from sift_parse.datamodel.asr_model_specs import (
    WHISPER_BASE,
...
from sift_parse.datamodel.backend_options import (
    EpubBackendOptions,
    HTMLBackendOptions,
    LatexBackendOptions,
    PdfBackendOptions,
...
from sift_parse.datamodel.base_models import (
    ConversionStatus,
...
from sift_parse.datamodel.document import ConversionResult, DoclingVersion
from sift_parse.datamodel.pipeline_options import (
    AsrPipelineOptions,
...
from sift_parse.datamodel.pipeline_options_asr_model import InlineAsrOptions
from sift_parse.datamodel.settings import settings
...
from sift_parse.models.factories import (
    get_layout_factory,
    get_ocr_factory,
    get_table_structure_factory,
)
from sift_parse.models.factories.base_factory import BaseFactory
from sift_parse.utils.profiling import ProfilingItem
...
class _DefaultCommandGroup(typer.core.TyperGroup):
    """Route a bare ``sift_parse <source>`` invocation to the ``convert`` command.

    Historically the CLI exposed a single command, so Typer let users run
    ``sift_parse report.pdf`` without naming it. Adding a second command
    (``convert-remote``) would otherwise force ``sift_parse convert report.pdf``
    ...
    arguments to ``convert``. ``sift_parse --help`` still shows the command list.
    """
...
app = typer.Typer(
    ...
    help=(
        ...
        "(useful when calling sift_parse from an AI agent or script)."
    ),
    ...
    epilog=(
        ...
        "use `sift_parse convert-remote` and read `sift_parse convert-remote --help` "
    ...
    if value:
        ...
        def print_external_plugins(factory: BaseFactory, factory_name: str):
            ...
            for meta in factory.registered_meta.values():
                if not meta.module.startswith("sift_parse."):
                    table.add_row(
...
def export_documents(
    ...
    if export_chunks:
        ...
        from sift_parse_core.transforms.chunker.hybrid_chunker import (
            HybridChunker,
        )
        from sift_parse_core.transforms.chunker.tokenizer.huggingface import (
...
def convert(  # noqa: C901
    ...
    release_native_memory_every_n_pages: Annotated[
        ...
        typer.Option(
            ...
            help=(
                ...
                "using the threaded sift-parse-pdf backend."
            ),
        ),
    ] = 128,
    device: Annotated[
    ...
    # (pypdfium2 / sift_parse_pdf). Only local `convert` needs them.
    from sift_parse.backend.sift_parse_pdf_backend import (
        DoclingParseDocumentBackend,
        ThreadedDoclingParseDocumentBackend,
    )
    from sift_parse.backend.image_backend import ImageDocumentBackend
    from sift_parse.backend.mets_gbs_backend import MetsGbsDocumentBackend
    from sift_parse.backend.pdf_backend import PdfDocumentBackend
    from sift_parse.backend.pypdfium2_backend import PyPdfiumDocumentBackend
    from sift_parse.document_converter import (
        AudioFormatOption,
        DocumentConverter,
        EpubFormatOption,
    ...
    from sift_parse.pipeline.asr_pipeline import AsrPipeline
    from sift_parse.pipeline.vlm_pipeline import VlmPipeline
    ...
    if verbose == 0:
        ...
        if not quiet:
            ...
            logging.getLogger("sift_parse.pipeline.base_pipeline").setLevel(logging.INFO)
            logging.getLogger("sift_parse.document_converter").setLevel(logging.INFO)
    elif verbose == 1:
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        # imports above: sift_parse.pipeline.video_pipeline transitively pulls
        ...
        if has_video_source:
            from sift_parse.datamodel.pipeline_options import VideoPipelineOptions
            from sift_parse.document_converter import VideoFormatOption
            from sift_parse.pipeline.video_pipeline import VideoPipeline
            from sift_parse.utils.video_frame_sampling import VideoFrameSamplingMode
...
# below.
try:
    from sift_parse.cli.remote import register as _register_remote
except ImportError:
    _log.debug(
