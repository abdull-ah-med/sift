...
try:
    ...
    print("\nThe sift-parse-tools CLI requires additional dependencies.", file=sys.stderr)
...
from sift_parse.models.stages.ocr.easyocr_model import (
    _resolve_easyocr_recognition_models,
)
from sift_parse.models.stages.ocr.rapid_ocr_model import _parse_rapidocr_model_spec
from sift_parse.models.utils.hf_model_download import download_hf_model
...
def download(
    ...
    if quiet:
        ...
        console.print(
            ...
            "Using the CLI:",
            f"`sift_parse --artifacts-path={output_dir} FILE`",
            "\n",
            "Using Python: see the documentation at <https://docling-project.github.io/sift_parse/usage>.",
        )
...
def download_hf_repo(
    ...
    for item in models:
        ...
        download_hf_model(
            repo_id=item,
            # would be better to reuse "repo_cache_folder" property: https://github.com/docling-project/docling/blob/main/sift_parse/datamodel/pipeline_options_vlm_model.py#L76
