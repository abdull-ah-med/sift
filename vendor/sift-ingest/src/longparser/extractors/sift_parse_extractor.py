...
from sift_parse_core.transforms.chunker import HierarchicalChunker
...
try:
    from sift_parse_core.types.doc import TitleItem
except ImportError:
    TitleItem = None  # Fallback for older sift_parse versions
...
class SiftParseExtractor(BaseExtractor):
    """
                    ...
                    if latex_eqs:
                        ...
                        if len(formula_blocks) != len(latex_eqs):
                            logger.warning(
                                ...
                                f"extracted={len(latex_eqs)}, sift_parse={len(formula_blocks)}. "
                                f"Injected {injected}."
                            )
                        else:
                ...
                for page_no, page_obj in result.document.pages.items():
                    ...
                    for mbox in mfd_boxes:
                        ...
                        if not replaced:
                            ...
                            try:
                                from sift_parse_core.types.doc import TextItem as _TextItem, DocItemLabel as _DIL
                                new_item = _TextItem(
                                    label=_DIL.FORMULA,
                                    text=latex_text,
                                    prov=[],
