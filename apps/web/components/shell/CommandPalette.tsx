...
const JUMPS = [
  ...
  { value: "login", label: "Sign in", href: "/login" },
] as const;
...
export function CommandPalette({ open, onOpenChange }: CommandPaletteProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 pt-[20vh]">
      ...
      />
      <Command
        className="relative z-10 w-full max-w-lg overflow-hidden rounded-lg border border-[rgb(var(--sift-border-strong))] bg-[rgb(var(--sift-surface))] shadow-xl"
        label="Command menu"
      >
        <Command.Input
          value={query}
        ...
        <Command.List className="max-h-72 overflow-auto p-2">
          ...
          <Command.Group heading="Navigate" className="px-1 text-xs text-[rgb(var(--sift-text-muted))]">
            {JUMPS.map((item) => (
              <Command.Item
                key={item.value}
                value={item.value}
