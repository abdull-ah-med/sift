...
Usage::
    ...
    from sift_ingest.utils.lang_detect import detect_language, get_tesseract_langs
...
def detect_language(
    ...
    try:
        ...
        result = detect(text)
        lang, score = coerce_detect_result(result)

        if score < min_confidence:
            logger.info(
                "Language detection low confidence (%.2f for '%s'), defaulting to English",
                score,
                lang,
            )
            return "en", score
        ...
        logger.warning(
            ...
            "Install with: pip install 'fast-langdetect>=0.3,<1.0'"
        )
        return "en", 0.0
...
def coerce_detect_result(result: object) -> tuple[str, float]:
    """Normalize fast-langdetect 0.x dict and 1.x list-of-dict return shapes."""
    candidate: object = result
    if isinstance(result, list):
        candidate = result[0] if result else {}
    if isinstance(candidate, dict):
        lang = str(candidate.get("lang") or candidate.get("language") or "en")
        raw_score = candidate.get("score", candidate.get("confidence", 0.0))
        try:
...
def extract_sample_text(file_path: object, max_chars: int = 2000) -> str:
...
def _extract_pdf_sample(file_path: object, max_chars: int) -> str:
    """Extract a printable-byte sample from a PDF (no AGPL PDF libs)."""
    try:
