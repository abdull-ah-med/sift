"""Formula OCR adapter (absorbed from vendor latex_ocr; feature-flagged).

Enable with ``SIFT_FORMULA_OCR=1``. Production-hardened with pluggable
backends (pix2tex / UniMERNet), CPU-forced inference, and graceful
degradation when weights are unavailable.
"""
...
def formula_ocr_enabled() -> bool:
    """Return True when formula OCR backends may load."""
    return os.environ.get("SIFT_FORMULA_OCR", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
class UniMERNetBackend(LaTeXOCRBackend):
    ...
    def load(self) -> bool:
        try:
            ...
            model_dir = os.getenv("SIFT_UNIMERNET_MODEL_DIR", "")
            if not model_dir or not os.path.isdir(model_dir):
                logger.warning(
                    "UniMERNet model dir not found. "
                    "Set SIFT_UNIMERNET_MODEL_DIR to the checkpoint directory."
                )
                return False
...
class LaTeXOCR:
    ...
    def _ensure_loaded(self):
        ...
        if not formula_ocr_enabled():
            # Do not mark initialized — enablement may flip later in-process.
            self._available = False
            logger.debug("Formula OCR disabled (set SIFT_FORMULA_OCR=1 to enable)")
            return
...
class MFDBackend:
    ...
    Requires SIFT_MFD_MODEL_DIR pointing to a directory containing
    ...
    def _load(cls) -> "MFDBackend":
        inst = cls()
        if not formula_ocr_enabled():
            logger.debug("MFD disabled: SIFT_FORMULA_OCR not enabled")
            return inst
        model_dir = os.getenv("SIFT_MFD_MODEL_DIR", "").strip()
        if not model_dir:
            logger.debug("MFD disabled: SIFT_MFD_MODEL_DIR not set")
            return inst
